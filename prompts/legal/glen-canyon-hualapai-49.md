# Legal - Glen Canyon Dam 1995 FEIS Hualapai River Runners 1987 income share

**Answer:** `49%`  
**Highlightable?** Yes. On usbr.gov Chapter 3 PDF page **100** (printed p.**165**), selectable text objects include `49` and `percent` in the 1987 Hualapai River Runners income sentence.  
**Searchable distractors:** same-page **"about 33 percent"** of total **1991** income from river-based recreation; **"approximately one-fourth"** / **25%** of gross revenue from desert bighorn sheep hunt permits.  
**Secondary traps:** Table III-16 commercial / below-Diamond dollar figures from the prior page; vague “approximately half” paraphrase of the same 49% sentence.  
**Doc:** 1995 Glen Canyon Dam operations FEIS Chapter 3 recreation economics / Indian Tribes (Hualapai).

---

## Prompt (77 words)

In the 1995 final environmental impact statement on the operation of Glen Canyon Dam whose preferred alternative was the Modified Low Fluctuating Flow Alternative, the recreation discussion of Hualapai tribal enterprises notes that the tribe earned about 33 percent of its total 1991 income from river-based recreation activities. Setting aside that 1991 share, what exact percentage of the Hualapai's total gross income does the same discussion say Hualapai River Runners earned in 1987? Report the percentage only.

---

## Answer

`49%`

---

## Golden Trajectory

**Step 1 - Search:** `"Operation of Glen Canyon Dam" "Final Environmental Impact Statement" 1995 "Modified Low Fluctuating Flow"`  
**Step 1 - Fetch:** https://en.wikipedia.org/wiki/Glen_Canyon_Dam - Exact on-page clues: (a) body text *"The EIS completed March 21, 1995 cemented some restrictions on dam operations..."*; (b) References: *U.S. Bureau of Reclamation (1995). Operation of Glen Canyon Dam: Colorado River Storage Project, Arizona: final environmental impact statement*; (c) External links **"1995 Glen Canyon EIS"** → https://www.usbr.gov/uc/envdocs/eis/gc/gcdOpsFEIS.html.  
**Step 1 - Verify:** Page names the 1995 *Operation of Glen Canyon Dam* Final EIS and Reclamation host.  
**Step 1 - Filter:** Identify Reclamation as author; stay on recreation / tribal-enterprise material.

**Step 2 - Search:** From that Wikipedia page, open External links **"1995 Glen Canyon EIS"**.  
**Step 2 - Fetch:** https://www.usbr.gov/uc/envdocs/eis/gc/gcdOpsFEIS.html - Official FEIS chapter index.  
**Step 2 - Verify:** Lead-agency copy.  
**Step 2 - Filter:** Use only PDFs linked from this index.

**Step 3 - Search:** On the FEIS index, open **"Cover Sheet, Table Of Contents, Etc."** (`cov-con.pdf`).  
**Step 3 - Fetch:** https://www.usbr.gov/uc/envdocs/eis/gc/pdfs/Cov-con/cov-con.pdf - Preferred alternative is the **Modified Low Fluctuating Flow Alternative**.  
**Step 3 - Verify:** Matches MLFF constraint.  
**Step 3 - Filter:** Confirm correct FEIS before Chapter 3.

**Step 4 - Search:** Return to FEIS index, open **"Chapter 3"** PDF; text-search `33 percent` / `1991` / `Hualapai` / `river-based`.  
**Step 4 - Fetch:** https://www.usbr.gov/uc/envdocs/eis/gc/pdfs/Ch3/chap3-1.pdf - PDF p.**100** / printed p.**165**. Selectable prose: the tribe earned about **33 percent** of its total **1991** income from river-based recreation activities. Also nearby: approximately **one-fourth** of gross revenue from desert bighorn sheep hunt permits.  
**Step 4 - Verify:** Distractor **33%** (1991) confirmed in selectable text.  
**Step 4 - Filter:** Reject **33%** and **one-fourth** / **25%**. Stay on the same Hualapai River Runners discussion for the 1987 figure.

**Step 5 - Search:** Same page; text-search `1987` / `Hualapai River Runners` / `49 percent`.  
**Step 5 - Fetch:** Same Chapter 3 PDF, PDF p.**100** / printed p.**165**. Selectable sentence: **"In 1987, Hualapai River Runners earned 49 percent or approximately half of the Hualapai's total gross income."**  
**Step 5 - Verify:** Highlightable answer is **49%**.  
**Step 5 - Filter:** Record **49%**. Discard **33%** (1991), **one-fourth** / **25%**, and any Table III-16 dollar figures from the prior page.

---

## Verification Sources

1. https://en.wikipedia.org/wiki/Glen_Canyon_Dam - exact March 21, 1995 EIS prose; 1995 FEIS reference; External links "1995 Glen Canyon EIS"  
2. https://www.usbr.gov/uc/envdocs/eis/gc/gcdOpsFEIS.html - official FEIS chapter index  
3. https://www.usbr.gov/uc/envdocs/eis/gc/pdfs/Cov-con/cov-con.pdf - Modified Low Fluctuating Flow preferred alternative  
4. https://www.usbr.gov/uc/envdocs/eis/gc/pdfs/Ch3/chap3-1.pdf - PDF p.100 / printed p.165: selectable **49 percent** (1987 Hualapai River Runners); distractor **33 percent** (1991)  

---

## Failure justification

*(Await live model response.)*

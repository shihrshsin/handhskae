# Legal - Glen Canyon Dam 1995 FEIS Table III-13 Medium camping beaches

**Answer:** `102`  
**Searchable distractors:** `333` (1975 inventory); `226` (suitable beaches after decline).  
**Secondary traps:** `47` (Small totals); `90` (Large totals); `239` (grand total); parenthetical low-water add-ons.  
**Doc:** Same FEIS Chapter 3 / Table III-13 as the grand-total `239` stump - Medium column totals only.

---

## Prompt (78 words)

In the 1995 final environmental impact statement on the operation of Glen Canyon Dam whose preferred alternative was the Modified Low Fluctuating Flow Alternative, the recreation analysis cites a 1975 inventory of about 333 campsites and later states that suitable camping beaches above the new high-water zone had declined to 226 sites. On the reach-by-reach distribution table of camping beaches by size class, excluding parenthetical low-water-only counts, what number of Medium camping beaches does that table report in its totals row?

---

## Answer

`102`

---

## Golden Trajectory

**Step 1 - Search:** `"Operation of Glen Canyon Dam" "Final Environmental Impact Statement" 1995 "Modified Low Fluctuating Flow"`  
**Step 1 - Fetch:** https://en.wikipedia.org/wiki/Glen_Canyon_Dam - Exact on-page clues: (a) body text *"The EIS completed March 21, 1995 cemented some restrictions on dam operations..."*; (b) References: *U.S. Bureau of Reclamation (1995). Operation of Glen Canyon Dam: Colorado River Storage Project, Arizona: final environmental impact statement*; (c) External links **"1995 Glen Canyon EIS"** → https://www.usbr.gov/uc/envdocs/eis/gc/gcdOpsFEIS.html.  
**Step 1 - Verify:** Page names the 1995 *Operation of Glen Canyon Dam* Final EIS and Reclamation host.  
**Step 1 - Filter:** Identify Reclamation as author; stay on camping-beach material.

**Step 2 - Search:** From that Wikipedia page, open External links **"1995 Glen Canyon EIS"**.  
**Step 2 - Fetch:** https://www.usbr.gov/uc/envdocs/eis/gc/gcdOpsFEIS.html - Official FEIS chapter index.  
**Step 2 - Verify:** Lead-agency copy.  
**Step 2 - Filter:** Use only PDFs linked from this index.

**Step 3 - Search:** On the FEIS index, open **"Cover Sheet, Table Of Contents, Etc."** (`cov-con.pdf`).  
**Step 3 - Fetch:** https://www.usbr.gov/uc/envdocs/eis/gc/pdfs/Cov-con/cov-con.pdf - Preferred alternative is the **Modified Low Fluctuating Flow Alternative**.  
**Step 3 - Verify:** Matches MLFF constraint.  
**Step 3 - Filter:** Confirm correct FEIS before Chapter 3.

**Step 4 - Search:** Return to FEIS index, open **"Chapter 3"** PDF; text-search `333` / `226` / `camping beaches`.  
**Step 4 - Fetch:** https://www.usbr.gov/uc/envdocs/eis/gc/pdfs/Ch3/chap3-1.pdf - printed p.**156**. Selectable prose: about **333** campsites in the 1975 inventory; suitable beaches above the new high-water zone down to **226** sites.  
**Step 4 - Verify:** Those are the prompt's inventory / decline distractors, not the Medium totals-row value.  
**Step 4 - Filter:** Reject **333** and **226**. Proceed to the size-class distribution table.

**Step 5 - Search:** Same Chapter 3 PDF, turn to printed p.**158** (zero-text scan) and read **Table III-13**.  
**Step 5 - Fetch:** https://www.usbr.gov/uc/envdocs/eis/gc/pdfs/Ch3/chap3-1.pdf - printed p.**158**. Table III-13, "Distribution of camping beaches by reach" (Kearsley and Warren, 1992). Totals row: Small **47 (27)**; Medium **102 (10)**; Large **90 (0)**; Total **239 (37)**. Parentheses = additional beaches only at low water. Labels are on the official scanned table.  
**Step 5 - Verify:** Excluding parentheticals, Medium totals-row value is **102**.  
**Step 5 - Filter:** Record **102**. Discard **333**, **226**, **47**, **90**, **239**, and parenthetical **10**.

**Step 6 - Search:** `"Operation of Glen Canyon Dam" 1995 EIS filetype:pdf` / riversimulator full mirror; text-search `333` / `226`.  
**Step 6 - Fetch:** http://www.riversimulator.org/Resources/USBR/LTEP/1995EIS.pdf - finds **333** / **226**; does not return Medium totals **102** as selectable table text (table page is a scan).  
**Step 6 - Verify:** Confirms the Medium total comes from official table labels, not searchable prose.  
**Step 6 - Filter:** Retain **102**.

---

## Verification Sources

1. https://en.wikipedia.org/wiki/Glen_Canyon_Dam - exact March 21, 1995 EIS prose; 1995 FEIS reference; External links "1995 Glen Canyon EIS"  
2. https://www.usbr.gov/uc/envdocs/eis/gc/gcdOpsFEIS.html - official FEIS chapter index  
3. https://www.usbr.gov/uc/envdocs/eis/gc/pdfs/Cov-con/cov-con.pdf - Modified Low Fluctuating Flow preferred alternative  
4. https://www.usbr.gov/uc/envdocs/eis/gc/pdfs/Ch3/chap3-1.pdf - p.156 distractors **333** / **226**; p.158 Table III-13 Medium **102**  
5. http://www.riversimulator.org/Resources/USBR/LTEP/1995EIS.pdf - full mirror  

---

## Failure justification

*(Await live model response.)*

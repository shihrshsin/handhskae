...
**Answer:** `3.6`  
**Famous twins:** `2.3` (Deer Creek sites per mile in body text; that is the 1991 chart bar); `1.0` (corridor-wide average in the same sentence).  
**Secondary traps:** `1.1` / `0.7` (1991 Non-critical / Critical); `2.2` (1973 Deer Creek); `2.7` / `1.9` (1983 Non-critical / Total).  
**Observed model failure:** Answered `3.0` (OCR/misread of the raster **3.6** Deer Creek 1983 bar on Figure III-38; found the FEIS / figure callout and the prose twin **2.3** for 1991). Earlier browsing run also answered `3.1`.  
...
**Step 5 - Filter:** Lock **3.6**. Dump **2.3**, **1.0**, **2.2**, and the other 1983 bars (**0.9** Critical / **2.7** Non-critical / **1.9** Total). Also dump OCR near-misses like **3.0** / **3.1**.
...
The model answered **3.0**, after browsing the usbr.gov Chapter 3 FEIS and naming Figure III-38. The Deer Creek 1983 bar on that zero-text chart is **3.6**, not 3.0. It correctly recited the prose twin (**2.3** sites per mile for the 1991 inventory) but never recovered the true 1983 Deer Creek numeral from the image. An earlier browsing run made the same class of error with **3.1**.
...
- [x] Observed fail = chart misread (`3.0` / earlier `3.1` ≠ `3.6`)  

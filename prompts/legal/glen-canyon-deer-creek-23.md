# Legal - Glen Canyon Dam 1995 FEIS Deer Creek reach 2.3 sites per mile

**Answer:** `2.3`  
**Status:** Model solved (not a stump).  
**Highlightable?** Yes. On usbr.gov Chapter 3 PDF page **92** (printed p.**157**), selectable text objects include `Deer`, `Creek`, and `2.3` (sites per mile).  
**Searchable distractors:** corridor averages in the same paragraph: **1.0** / **0.7** / **1.1** per mile; large-campsite densities **0.20** / **0.51** per mile; nearby inundation prose **35%** / size-class **36%**.  
**Secondary traps:** Figure III-38 Deer Creek chart values **2.2** (1973) and **3.6** (1983) from prior image-only stumps (not this sites-per-mile prose).  
**Doc:** 1995 Glen Canyon Dam operations FEIS Chapter 3 recreation / camping-beach availability prose (Kearsley and Warren, 1993).

---

## Prompt (82 words)

In the 1995 final environmental impact statement on the operation of Glen Canyon Dam whose preferred alternative was the Modified Low Fluctuating Flow Alternative, the recreation discussion of camping-beach availability notes that campsites average 1.0 per mile overall, with critical reaches averaging 0.7 per mile and noncritical reaches averaging 1.1 per mile. Setting aside those corridor averages and any campsite-density chart values, what exact sites-per-mile figure does the same discussion assign to the Deer Creek reach (RM 131-139)? Report the number only.

---

## Answer

`2.3`

---

## Golden Trajectory

**Step 1 - Search:** `"Operation of Glen Canyon Dam" "Final Environmental Impact Statement" 1995 "Modified Low Fluctuating Flow"`  
**Step 1 - Fetch:** https://en.wikipedia.org/wiki/Glen_Canyon_Dam - Exact on-page clues: (a) body text *"The EIS completed March 21, 1995 cemented some restrictions on dam operations..."*; (b) References: *U.S. Bureau of Reclamation (1995). Operation of Glen Canyon Dam: Colorado River Storage Project, Arizona: final environmental impact statement*; (c) External links **"1995 Glen Canyon EIS"** → https://www.usbr.gov/uc/envdocs/eis/gc/gcdOpsFEIS.html.  
**Step 1 - Verify:** Page names the 1995 *Operation of Glen Canyon Dam* Final EIS and Reclamation host.  
**Step 1 - Filter:** Identify Reclamation as author; stay on recreation / camping-beach material.

**Step 2 - Search:** From that Wikipedia page, open External links **"1995 Glen Canyon EIS"**.  
**Step 2 - Fetch:** https://www.usbr.gov/uc/envdocs/eis/gc/gcdOpsFEIS.html - Official FEIS chapter index.  
**Step 2 - Verify:** Lead-agency copy.  
**Step 2 - Filter:** Use only PDFs linked from this index.

**Step 3 - Search:** On the FEIS index, open **"Cover Sheet, Table Of Contents, Etc."** (`cov-con.pdf`).  
**Step 3 - Fetch:** https://www.usbr.gov/uc/envdocs/eis/gc/pdfs/Cov-con/cov-con.pdf - Preferred alternative is the **Modified Low Fluctuating Flow Alternative**.  
**Step 3 - Verify:** Matches MLFF constraint.  
**Step 3 - Filter:** Confirm correct FEIS before Chapter 3.

**Step 4 - Search:** Return to FEIS index, open **"Chapter 3"** PDF; text-search `Deer Creek` / `0.7 per mile` / `1.1 per mile` / `sites per mile`.  
**Step 4 - Fetch:** https://www.usbr.gov/uc/envdocs/eis/gc/pdfs/Ch3/chap3-1.pdf - PDF p.**92** / printed p.**157**. Selectable prose: campsites average **1.0** per mile; critical reaches **0.7** per mile; noncritical reaches **1.1** per mile; critical reaches have **0.20** large campsites per mile versus **0.51** in noncritical reaches.  
**Step 4 - Verify:** Distractor averages confirmed in the text layer.  
**Step 4 - Filter:** Reject **1.0** / **0.7** / **1.1** / **0.20** / **0.51**. Continue in the same paragraph for Deer Creek.

**Step 5 - Search:** Same page / same paragraph; text-select `Deer Creek` / `131-139` / `2.3`.  
**Step 5 - Fetch:** Same Chapter 3 PDF, PDF p.**92**. Selectable sentence: **"Deer Creek reach (RM 131-139) has more sites per mile than any other river reach at 2.3 sites per mile."**  
**Step 5 - Verify:** Highlightable answer is **2.3**.  
**Step 5 - Filter:** Record **2.3**. Discard corridor averages and any Figure III-38 chart values (**2.2** / **3.6**) that are not this sites-per-mile prose.

---

## Verification Sources

1. https://en.wikipedia.org/wiki/Glen_Canyon_Dam - exact March 21, 1995 EIS prose; 1995 FEIS reference; External links "1995 Glen Canyon EIS"  
2. https://www.usbr.gov/uc/envdocs/eis/gc/gcdOpsFEIS.html - official FEIS chapter index  
3. https://www.usbr.gov/uc/envdocs/eis/gc/pdfs/Cov-con/cov-con.pdf - Modified Low Fluctuating Flow preferred alternative  
4. https://www.usbr.gov/uc/envdocs/eis/gc/pdfs/Ch3/chap3-1.pdf - PDF p.92 / printed p.157: selectable **Deer Creek ... 2.3 sites per mile**  

---

## Failure justification

The model answered **2.3** correctly. The sites-per-mile prose is selectable on usbr.gov Chapter 3 and was recovered from the Deer Creek sentence, so this item does not stump. Abandoned as a live stump candidate.
